...
@HiltViewModel
class DydxReceiptBridgeFeeViewModel @Inject constructor(
    ...
    private fun createViewState(
        ...
        val bridgeFee = transferInput?.summary?.bridgeFee
        return DydxReceiptBaseFeeView.ViewState(
            localizer = localizer,
            feeType = "Bridge",
            feeFont = if (bridgeFee == null) {
                null
            } else if (bridgeFee > 0.0) {
                DydxReceiptBaseFeeView.FeeFont.Number(
                    formatter.dollar(bridgeFee, 2) ?: "",
                )
            } else {
