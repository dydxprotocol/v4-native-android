...
class DydxPortfolioFundingsViewModel @Inject constructor(
    ...
    val state: Flow<DydxPortfolioFundingsView.ViewState?> = abacusStateManager.state.marketSummary
        .map {
            createViewState(it)
        }
        .distinctUntilChanged()

    private fun createViewState(marketSummary: PerpetualMarketSummary?): DydxPortfolioFundingsView.ViewState {
